...
import type { Router, Request, Response } from 'express'    // types for TS
...
import type { UserItem } from '../data/types.js'             // user item shape

const router: Router = express.Router()                      // make router

// GET /api/users -> list usernames + userId
router.get('/', async (req: Request, res: Response<void | Array<{ username: string; userId: string }>>) => {
  try {
    ...
    const items = (out.Items ?? []) as UserItem[]
...
function validateJwt(authHeader: string | undefined): { userId: string } | null {
  // header format: "Bearer <token>"
  if (!authHeader) return null // no header

  const maybeToken = authHeader.split(' ')[1]?.trim() // get token part
  if (!maybeToken) return null // no token

  try {
    // verify token using the same secret as when signing
    const decoded = jwt.verify(maybeToken, process.env.JWT_SECRET || '') as { userId?: unknown }
...
router.delete('/:userId', async (req: Request<{ userId: string }>, res: Response<void>) => {
  try {
...
router.get('/me', async (req: Request, res: Response) => {
  ...
  const item = out.Item as Pick<UserItem, 'sk' | 'username'>
