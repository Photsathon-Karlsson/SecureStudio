...
function auth(req: Request, res: Response, next: NextFunction) {
  ...
  if (!authHeader) return res.status(401).json({ error: "missing authorization header" }) // no header

  // format should be: "Bearer <token>"
  const token = authHeader.split(" ")[1]?.trim() // get token after "Bearer"
  if (!token) return res.status(401).json({ error: "missing token" }) // no token found
  ...
  try {
    ...
    ;(req as any).user = decoded // save user info in request
...
app.post("/api/register", (req, res) => {
  ...
  const newUser: User = { id: nextId++, username, password } // make new user
...
app.get("/api/users/me", auth, (req, res) => { // create endpoint
  const decoded = (req as any).user as { sub: number, username: string } // get data from middleware
  if (!decoded?.sub) return res.status(401).json({ error: "bad token" }) // token missing/bad
...
app.get("/", (req, res) => { // when user go to homepage
  res.send("Hello SecureStudio!") // send back simple text
})

app.listen(1444, () => { // start server on port 1444
